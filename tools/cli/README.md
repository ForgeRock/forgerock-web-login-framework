...
### `ping-lf generate header <Name>`

### `ping-lf generate footer <Name>`

Scaffolds a new custom component under `experimental/custom/callbacks/<slug>/`, `experimental/custom/stages/<slug>/`, `experimental/custom/headers/<slug>/`, or `experimental/custom/footers/<slug>/`. The Vite plugin watches these directories during `pnpm dev` and regenerates `custom-registry.ts` automatically on add/remove/change.
...
# Add a branded page header / footer
ping-lf generate header "Corporate Header"
ping-lf generate footer "Corporate Footer"
```
...
For `generate header` / `generate footer`, the scaffolded directory contains the component plus Storybook files (`.stories.js` + `.story.svelte`) — headers and footers take no props, so there are no utilities or mocks:

```
experimental/custom/headers/corporate-header/
├── corporate-header.svelte       # component implementation — edit this
├── corporate-header.stories.js   # Storybook stories
└── corporate-header.story.svelte # Storybook story wrapper
```
...
| `generate_header`   | Scaffold a custom header component   |
| `generate_footer`   | Scaffold a custom footer component   |
| `update`            | Update the framework version         |
| `list_releases`     | List available releases from GitHub  |

For `generate_callback`, `generate_stage`, `generate_header`, `generate_footer`, and `update`, pass an absolute path via the `directory` parameter to target a specific project root. If omitted, the server's working directory is used.
