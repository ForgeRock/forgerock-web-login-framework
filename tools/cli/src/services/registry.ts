...
export const parseComponentHeader = (
  ...
  // Mirrors core/journey/_utilities/registry/registry.ts: "__proto__" as an
  // object-literal key sets the prototype instead of defining an own property,
  // so the generated Record would silently lose the entry; "constructor" is
  // inherited-but-shadowable and kept reserved for symmetry.
  ...
  return Effect.succeed({ type: rawType as ComponentType, name });
};
