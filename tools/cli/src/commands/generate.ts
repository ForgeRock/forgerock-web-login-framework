...
export const scaffoldUserDirectory = (targetDir: string) =>
  Effect.gen(function* () {
    ...
    yield* efs.makeDirectory(callbackDir, { recursive: true }).pipe(
      Effect.mapError(
        (cause) =>
          new FileSystemError({
    ...
    yield* efs.makeDirectory(stageDir, { recursive: true }).pipe(
      Effect.mapError(
        (cause) =>
          new FileSystemError({
            operation: 'makeDirectory',
    ...
    if (!callbackExists) {
      yield* copyTemplateDir(p.join(templateDir, 'user/callback/demo-callback'), demoCallbackDir);
    }

    const stageExists = yield* efs.exists(demoStageDir);
    if (!stageExists) {
      yield* copyTemplateDir(p.join(templateDir, 'user/stage/demo-stage'), demoStageDir);
    }
...
const copyTemplateDir = (sourceDir: string, targetDir: string) =>
  Effect.gen(function* () {
    ...
    yield* efs.makeDirectory(targetDir, { recursive: true }).pipe(
      Effect.mapError(
        (cause) =>
          new FileSystemError({
            operation: 'makeDirectory',
...
export const scaffoldCi = (targetDir: string) =>
  Effect.gen(function* () {
    ...
    yield* efs.makeDirectory(workflowDir, { recursive: true }).pipe(
      Effect.mapError(
        (cause) =>
          new FileSystemError({
            operation: 'makeDirectory',
    ...
    Effect.gen(function* () {
      ...
      if (pkgJsonExists) {
        ...
        if (pkgJson.scripts?.['ping-law']) {
          ...
          yield* efs.writeFileString(pkgJsonPath, JSON.stringify(pkgJson, null, 2) + '\n');
        }
      }
      ...
      yield* efs
        .makeDirectory(p.dirname(registryPath), { recursive: true })
