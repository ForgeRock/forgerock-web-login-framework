...
describe("scaffoldUserDirectory", () => {
  ...
  it("creates user/stage and user/callback directories", () =>
    scaffoldUserDirectory(targetDir).pipe(
      Effect.andThen(() => {
        expect(
          fs.existsSync(path.join(targetDir, "user", "callback")),
        ).toBe(true);
        expect(
          fs.existsSync(path.join(targetDir, "user", "stage")),
        ).toBe(true);
      }),
  ...
  it("copies demo callback component from templates", () =>
    scaffoldUserDirectory(targetDir).pipe(
      Effect.andThen(() => {
        ...
        expect(
          fs.existsSync(path.join(demoDir, "component.svelte")),
        ).toBe(true);
        expect(
          fs.existsSync(path.join(demoDir, "utility.ts")),
        ).toBe(true);
      }),
      Effect.provide(NodeContext.layer),
      Effect.runPromise,
  ...
  it("does NOT overwrite existing user content", () => {
    ...
    fs.writeFileSync(
      path.join(existingDir, "component.svelte"),
      "<!-- custom content -->",
    );
