...
describe("copyWithExclusions", () => {
  ...
  beforeEach(() => {
    ...
    fs.writeFileSync(
      path.join(sourceDir, ".github/workflows/ci.yml"),
      "name: CI",
    );
    fs.writeFileSync(
      path.join(sourceDir, ".changeset/config.json"),
      "{}",
    );
  ...
  it("skips excluded files", () =>
    copyWithExclusions(sourceDir, targetDir).pipe(
      Effect.andThen(() => {
        ...
        expect(
          fs.existsSync(path.join(targetDir, ".changeset/config.json")),
        ).toBe(false);
      }),
      Effect.provide(NodeContext.layer),
      Effect.runPromise,
  ...
  it("never overwrites /user directory", () => {
    ...
    fs.writeFileSync(
      path.join(targetDir, "user/callback/my-cb/component.svelte"),
      "custom",
    );
