 ...
 *
 * SvelteKit's default prod handleError replaces a raw Error.message with "Internal Error"
 * on the rendered page. +page.svelte re-throws selectRegistryEntry's Error through
 * SvelteKit's error(500, message) helper, so the full diagnostic must survive to
 * page.error.body.message and render on +error.svelte. The message names the env var
 * and the configured value, proving which combination triggered the throw.
 * Isolated so the throwing env doesn't affect the shared webServer used by every
...
test('Unknown custom header name throws at page start and renders the 500 error page', async ({
  ...
  await expect(page.getByText('PUBLIC_CUSTOM_HEADER_NAME is set to "GhostName"')).toBeVisible();
  await expect(page.getByText(/Available names: DemoHeader/)).toBeVisible();
});
