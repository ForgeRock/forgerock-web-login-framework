...
test('Inline widget prints scripted text output by default', async ({ page }) => {
  ...
  // The inline page passes no text-output style rules, and the journey's entry node
  // is the scripted decision itself, so the callbacks render as soon as the widget
  // starts the journey on mount — no credentials step in between.
