...
test('Classic ReCaptcha visible submits token from grecaptcha (inline)', async ({ page }) => {
...
test('ReCaptchaEnterpriseCallback visible renders via enterprise namespace (inline)', async ({
  page,
}) => {
...
test('ReCaptchaEnterpriseCallback invisible executes with action and submits token (inline)', async ({
  page,
}) => {
  await stubGrecaptchaEnterprise(page);
  const submitted = [];
  ...
  const { clickButton, navigate } = asyncEvents(page);
  await navigate('widget/inline?journey=TEST_Login&captchaMode=invisible&recaptchaAction=LOGIN');
